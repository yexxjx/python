...
engsentence=input("영문 문장> ")
print(engsentence.upper())
print(engsentence.lower())
...
letter="Hello, Python World!".find("Python")
print(letter)
...
db_id=input("아이디 입력> ")
db_pw=input("비밀번호 입력> ")
if str(db_id)=="user01" and str(db_pw)=="pass12":
        print("성공")
else: print("실패")
...
boy=[1, 3]
girl=[2, 4]
resident=input("주민번호 뒷자리 첫 번째 숫자> ")
if int(resident) in boy:
    print("남자")
elif int(resident) in girl:
    print("여자")
else: print("")
...
totalscore=input("점수> ")
if int(totalscore)>=90:
        print("A")
elif int(totalscore)>=80:
        print("B")
elif int(totalscore)>=70:
        print("C")
else: print("F")

# 문제 11: 할인 적용 결제 금액
# 구매 금액을 입력받아 10만원 이상이면 20% 할인, 5만원 이상이면 10% 할인, 그 미만은 할인이 없는 최종 결제 금액을 출력하시오.
purchase=input("구매 금액> ")
if float(purchase)>=100000:
        print(float(purchase)*0.80)
elif float(purchase)>=50000:
        print(float(purchase)*0.90)
else: print(purchase)

# 문제 12: 가위바위보 결과
# 플레이어1과 플레이어2가 각각 낸 가위(0), 바위(1), 보(2) 숫자를 입력받아 플레이어1 기준으로 "승리", "패배", "무승부" 중 하나를 출력하시오.
player1=input("player1 가위(0), 바위(1), 보(2)> ")
player2=input("player2 가위(0), 바위(1), 보(2)> ")
if int(player1)==int(player2):
        print("무승부")
elif (int(player1)==0 and int(player2)==2) or (int(player1)==1 and int(player2)==0) or (int(player1)==2 and int(player2)==1):
        print("승리")
else: print("패배")
...
if str(nickname).find("관리자")==0:
        print("[{}]님 환영합니다.".format("관리자"))
else: print("[", nickname ,"]님 안녕하세요.")